...
import { CAR_CONSTANTS, DEFAULT_TUNING, TORQUE_CURVE, CONTROLS } from '../constants';
...
const GameCanvas: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      switch (e.key) {
        case CONTROLS.FORWARD: inputsRef.current.forward = true; break;
        case CONTROLS.LEFT: inputsRef.current.left = true; break;
        case CONTROLS.RIGHT: inputsRef.current.right = true; break;
        case CONTROLS.BRAKE: inputsRef.current.brake = true; break;
        case CONTROLS.E_BRAKE: inputsRef.current.eBrake = true; break;
        case CONTROLS.CLUTCH: inputsRef.current.clutch = true; break;
        case CONTROLS.GEAR_UP: inputsRef.current.gearUp = true; break;
        case CONTROLS.GEAR_DOWN: inputsRef.current.gearDown = true; break;
      }
    };

    const handleKeyUp = (e: KeyboardEvent) => {
      switch (e.key) {
        case CONTROLS.FORWARD: inputsRef.current.forward = false; break;
        case CONTROLS.LEFT: inputsRef.current.left = false; break;
        case CONTROLS.RIGHT: inputsRef.current.right = false; break;
        case CONTROLS.BRAKE: inputsRef.current.brake = false; break;
        case CONTROLS.E_BRAKE: inputsRef.current.eBrake = false; break;
        case CONTROLS.CLUTCH: inputsRef.current.clutch = false; break;
        case CONTROLS.GEAR_UP: inputsRef.current.gearUp = false; break;
        case CONTROLS.GEAR_DOWN: inputsRef.current.gearDown = false; break;
      }
    };
  ...
  useEffect(() => {
    ...
    const updatePhysics = (dt: number) => {
      ...
      if (Math.abs(targetSteer - car.steeringAngle) < steerSpeed) {
        ...
        car.steeringAngle += Math.sign(targetSteer - car.steeringAngle) * steerSpeed;
      }

      // --- 3. Velocity Decomposition (Local Space) ---
      // Heading Vector (Forward direction of car body)
      const headX = Math.cos(car.heading);
      const headY = Math.sin(car.heading);
      
      // Side Vector (Perpendicular to heading)
      const sideX = -headY;
      const sideY = headX;

      // Project world velocity onto local axes
      // vLong = Forward velocity
      // vLat = Sideways velocity (Drifting component)
      ...
      // Calculate Slip Angle (for UI and physics)
      car.slipAngle = Math.abs(vLong) > 1 ? Math.atan2(vLat, Math.abs(vLong)) : 0;
      ...
      const engineTorque = inputs.forward ? (t.maxTorque * torqueCurveFactor) : 0;
      ...
      if (isDeclutched) {
        ...
        const availableForce = (engineTorque * effectiveRatio) / c.wheelRadius;
        ...
        if (!inputs.forward && car.rpm > t.idleRPM) {
             ...
             driveForce -= (engineBrakeTorque * effectiveRatio) / c.wheelRadius * Math.sign(vLong);
        }
      ...
      // Robust Low Speed Braking / Stopping
      if (inputs.brake) {
        const brakingDecel = t.brakingForce / t.mass;
        const timeToStop = Math.abs(vLong) / brakingDecel;
        
        if (timeToStop < dt) {
          // If we can stop within this frame, apply exact force to stop and zero velocity
        ...
        // Rolling Resistance
        // Apply friction only if moving
        if (Math.abs(vLong) > 0.1) {
            longForce -= Math.sign(vLong) * (c.rollingResistance + (t.dragCoefficient * vLong * vLong));
        } else if (!inputs.forward) {
             // Stop drift/creep at tiny speeds
             longForce = -(vLong * t.mass) / dt;
        }
      }

      // --- LATERAL FORCES (DRIFTING) ---
      // Calculate Grip Limit (Traction Circle Radius)
      const normalLoad = t.mass * 9.81;
      const maxLatGrip = normalLoad * t.tireGrip;
      const currentGripLimit = inputs.eBrake ? maxLatGrip * 0.2 : maxLatGrip;
      
      // Calculate Desired Lateral Force based on stiffness (Hooke's law for tires)
      // This is force, not impulse. 
      const corneringStiffness = 30.0 * t.mass; // N per m/s of slip
      let latForce = -vLat * corneringStiffness;

      // Check against Grip Limit
      const latForceMag = Math.abs(latForce);
      car.gripRatio = latForceMag / (currentGripLimit || 1); // Avoid div by zero

      if (latForceMag > currentGripLimit) {
        // Tire saturation / Sliding
      ...
      // --- 6. Suspension Physics (Visual Only) ---
      const accelX = longForce / t.mass; // Forward accel
      const accelY = latForce / t.mass;  // Lateral accel
      ...
      let targetAngularVel = (vLong * Math.tan(car.steeringAngle)) / wheelBase;
      if (vLong < -1) targetAngularVel = -targetAngularVel; 

      if (car.isDrifting) {
          // When drifting, user has less direct control over yaw rate, it becomes momentum based
          // Blending steer control with rotational inertia
          car.angularVelocity = car.angularVelocity * 0.98 + targetAngularVel * 0.02;
      } else {
          car.angularVelocity = targetAngularVel;
      }
      
      car.heading += car.angularVelocity * dt;

      // --- 8. Effects (Skidmarks & Particles) ---
      // Add skidmarks if drifting OR if grip ratio is high (near limit)
      if ((car.isDrifting || car.gripRatio > 0.8) && Math.abs(car.speed) > 2) {
        ...
        const skidPoint = { x: 0, y: 0, alpha: intensity * 0.6, life: 1.0 };
        
        skidmarksLeftRef.current.push({ ...skidPoint, x: rlX, y: rlY });
        skidmarksRightRef.current.push({ ...skidPoint, x: rrX, y: rrY });
      ...
      skidmarksLeftRef.current = skidmarksLeftRef.current.filter(p => p.life > 0);
      skidmarksRightRef.current = skidmarksRightRef.current.filter(p => p.life > 0);
    ...
    const draw = () => {
      ...
      // --- Camera Logic ---
      // Follow the Visual Body Center to prevent rubber-banding
      const susPitchOffset = car.pitch * PPM; // Local X Offset (Pitch tilts body forward/back)
      const susRollOffset = -car.roll * PPM;  // Local Y Offset (Roll tilts body left/right)
      ...
      // --- Draw Skidmarks ---
      ctx.lineWidth = 0.3 * PPM; // ~30cm wide skidmarks
      const drawTrail = (trail: TrailPoint[]) => {
        ...
        for (let i = 0; i < trail.length - 1; i++) {
           const p1 = trail[i];
           const p2 = trail[i+1];
           if (Math.abs(p1.x - car.x) > 100 || Math.abs(p1.y - car.y) > 100) continue;
      ...
      // Calculate Visual Offsets based on PPM
      const localSusX = -car.roll * PPM; 
      const localSusY = car.pitch * PPM;

      // Shadow
      ctx.fillStyle = 'rgba(0,0,0,0.5)';
      // Car is approx 4.8m x 2m
      const shadowLen = 5.0 * PPM;
      const shadowWidth = 2.2 * PPM;
      ctx.fillRect(-shadowLen/2, -shadowWidth/2, shadowLen, shadowWidth);

      // Wheels
      // Using accurate physics placement: 2.6m wheelbase
      const wheelOffsetFront = (CAR_CONSTANTS.wheelBase / 2) * PPM;
      const wheelOffsetRear = -(CAR_CONSTANTS.wheelBase / 2) * PPM;
      const wheelOffsetSide = 0.9 * PPM; // 1.8m track width
      ...
      // Rear Wheels
      ctx.fillRect(wheelOffsetRear - wheelDiameter/2, -wheelOffsetSide - wheelWidth/2, wheelDiameter, wheelWidth);
      ctx.fillRect(wheelOffsetRear - wheelDiameter/2, wheelOffsetSide - wheelWidth/2, wheelDiameter, wheelWidth);
      ...
      ctx.fillRect(-wheelDiameter/2, -wheelWidth/2, wheelDiameter, wheelWidth);
      ctx.restore();
      ...
      // Body (Suspension Affected)
      ctx.translate(localSusY, localSusX); 
      
      // Car Body Dimensions (e.g., M3: 4.7m length, 1.85m width)
      ...
      ctx.beginPath();
      ctx.roundRect(-bodyLen/2, -bodyWidth/2, bodyLen, bodyWidth, 0.2 * PPM);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.arc(bodyLen/2 - 0.1*PPM, -bodyWidth/2 + 0.4*PPM, lightSize, 0, Math.PI*2);
      ctx.arc(bodyLen/2 - 0.1*PPM, bodyWidth/2 - 0.4*PPM, lightSize, 0, Math.PI*2);
      ctx.fill();
  ...
  return (
    <div className="relative w-full h-full">
      <canvas 
        ...
        className="block"
      />
      <Dashboard carState={uiState} tuning={tuningRef.current} />
