...
export class AudioEngine {
  ...
  setConfiguration(cylinders: number, exhaustOpenness: number, backfireAggression: number, turboIntensity: number) {
    ...
    if (this.distortion && Math.abs(drive - this.lastDistortionAmount) > 1) {
  ...
  setSpatial(distanceMeters: number, relativeVelocityMS: number, panX: number) {
      if (!this.ctx || !this.masterGain || !this.panner || !this.mufflerFilter) return;
      ...
      const finalVol = this.baseVolume * attenuation;
      ...
      const dopplerFactor = (speedOfSound) / (speedOfSound + relativeVelocityMS);
  ...
  update(rpm: number, load: number, driftIntensity: number = 0) {
    ...
    if (this.currentCylinders === 4) {
      subRatio = 0.5; harmRatio = 1.5; if (this.subOsc) this.subOsc.type = 'square'; fmDepth = 0.2;
    } else if (this.currentCylinders === 6) {
      subRatio = 0.5; harmRatio = 2.0; if (this.subOsc) this.subOsc.type = 'sine'; fmDepth = 0.35;
    } else if (this.currentCylinders === 8) {
      subRatio = 0.5; harmRatio = 4.0; if (this.subOsc) this.subOsc.type = 'sawtooth'; fmDepth = 0.6; 
    } else if (this.currentCylinders >= 10) {
      subRatio = 1.0; harmRatio = 1.5; if (this.subOsc) this.subOsc.type = 'triangle'; fmDepth = 0.2; 
    }

    this.mainOsc?.frequency.setTargetAtTime(fundamental, now, 0.02);
    this.subOsc?.frequency.setTargetAtTime(fundamental * subRatio, now, 0.02);
    this.harmonicsOsc?.frequency.setTargetAtTime(fundamental * harmRatio, now, 0.02);
    ...
    if (rpm < 1100 && this.currentCylinders > 4) {
       const lopeAmount = (1 - (rpm/1100)) * (this.exhaustOpenness * 0.5 + 0.1);
    ...
    if (this.turboOsc && this.turboGain) {
      if (this.turboIntensity > 0.01) {
         ...
         const turboVol = this.turboIntensity * (rpmFactor * 0.6 + loadFactor * 0.4) * 0.3;
  ...
  triggerShift(isDownshift: boolean) {
    ...
    if (isDownshift) {
        ...
        if (this.exhaustOpenness > 0.3) {
             setTimeout(() => { this.createPop(0.3); }, 80);
        }
    }
  }
  ...
  private createPop(volume: number) {
      ...
      gain.gain.exponentialRampToValueAtTime(0.01, this.ctx.currentTime + duration * 0.8);
  ...
  private createNoiseBurst(duration: number, peakVolume: number, filterFreq: number) {
    if (!this.ctx || !this.masterGain) return;
    const bufferSize = this.ctx.sampleRate * duration;
    const buffer = this.ctx.createBuffer(1, bufferSize, this.ctx.sampleRate);
    const data = buffer.getChannelData(0);
    ...
    noiseGain.gain.linearRampToValueAtTime(peakVolume, this.ctx.currentTime + 0.005);
    ...
    noise.start();
    setTimeout(() => { noise.stop(); noise.disconnect(); noiseGain.disconnect(); noiseFilter.disconnect(); }, duration * 1000 + 100);
  }
